...
def count_layers_per_tenant(pageserver: NeonPageserver, timelines: Iterable[Tuple[TenantId, TimelineId]]) -> Dict[TenantId, int]:
...
def pgbench_init_tenant(layer_size: int, scale: int, env: NeonEnv, pg_bin: PgBin) -> Tuple[TenantId, TimelineId]:
...
def finish_tenant_creation(env: NeonEnv, tenant_id: TenantId, timeline_id: TimelineId, min_expected_layers: int) -> Lsn:
...
def test_dube_and_fast_growing_tenant(neon_env_builder: NeonEnvBuilder, pg_bin: PgBin, order: EvictionOrder):
    """
    ...
    ratios = []
    for i, ((tenant_id, timeline_id), _scale) in enumerate(timelines):
    ...
    assert len(ratios) == 4, "rest of the assertions expect 3 + 1 timelines, ratios, scales, all in order"
    log.info(f"{ratios}")
    ...
    if order == EvictionOrder.ABSOLUTE_ORDER:
        ...
        for (i, ratio) in enumerate(ratios):
            if i > 0 and ratios[i - 1] == 1.0:
                assert ratio == 1.0, "following after first tenant which did not lose must have all of the layers"
            elif ratio < 1.0:
                losers += 1
        ...
        assert all(map(lambda x: x == ratios[0], ratios[:3])), "scale=1 tenants should lose layers equally"
